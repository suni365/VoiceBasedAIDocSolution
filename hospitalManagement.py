...
# Using a new DB name to ensure the new columns (medicine_fees, remarks) are created correctly
conn = sqlite3.connect('clinic_v3.db', check_same_thread=False)
cursor = conn.cursor()

# Updated table schema with Medicine Fees and Remarks
cursor.execute('''
...
def send_whatsapp_msg(phone, name, patient_id, total_fees):
    """Generates a WhatsApp URL for patient summary."""
...
def get_daily_stats():
    """Fetches daily summary statistics including new medicine fees."""
...
def register_patient():
    ...
    with st.form("reg_form", clear_on_submit=True):
        ...
        with col1:
            ...
            phone = st.text_input("Mobile Number (with country code if not India)")
...
def search_records():
    ...
    if query:
        df = pd.read_sql("""
            SELECT * FROM patients WHERE name LIKE ? OR phone LIKE ? OR pid LIKE ?
            ...
            for _, row in df.iterrows():
                with st.expander(f"👤 {row['name']} - {row['visit_date']}"):
                    ...
                    if row['report_path'] and os.path.exists(row['report_path']):
                        st.download_button("Download Report", open(row['report_path'], 'rb'), file_name=os.path.basename(row['report_path']))
